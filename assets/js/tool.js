...
(function(){
  ...
  function renderImage(root,tool){ root.innerHTML=''; var file=input('file'), a=input('number','1200'), b=input('number','630'), color=input('color','#0f766e'), preview=el('div',{class:'image-preview'}), out=box('Result'); file.accept='image/*'; function showSvg(){ var svg='<svg xmlns="http://www.w3.org/2000/svg" width="'+a.value+'" height="'+b.value+'"><rect width="100%" height="100%" fill="'+color.value+'"/><text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" font-family="Arial" font-size="48" fill="#fff">ToolVanta</text></svg>'; preview.innerHTML=svg; out.box.innerHTML=''; var img=document.createElement('img'); img.alt='Generated SVG'; img.src='data:image/svg+xml;charset=utf-8,'+encodeURIComponent(svg); out.box.appendChild(img); } function runFile(){ if(!file.files[0]){showSvg();return;} var fr=new FileReader(); fr.onload=function(){ if(tool.type==='image-base64'){out.box.textContent=fr.result;} preview.innerHTML='<img alt="Preview" src="'+fr.result+'">'; }; fr.readAsDataURL(file.files[0]); } [file,a,b,color].forEach(function(x){x.addEventListener('input',runFile);x.addEventListener('change',runFile);}); if(tool.type==='svg-optimizer') return renderDev(root,tool); if(tool.type==='gradient-generator'){ preview.className='gradient-preview'; function grad(){ var css='linear-gradient(135deg, '+color.value+', #8a1538)'; preview.style.background=css; out.box.textContent='background: '+css+';'; } color.addEventListener('input',grad); root.appendChild(el('div',{class:'tool-form'},field('First color',color),preview,out.wrap)); grad(); return; } root.appendChild(el('div',{class:'tool-form'},field('Image file',file),el('div',{class:'inline-grid'},field('Width',a),field('Height',b)),field('Color',color),preview,out.wrap)); showSvg(); }
